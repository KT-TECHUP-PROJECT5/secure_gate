...
- [ ] C파트가 전달할 Trivy 원본 JSON 파일 경로 확정
- [ ] EC2 OneAgent의 `environment=staging`, `service=secure-gate` 태그 확인
- [ ] Dynatrace Synthetic HTTP Monitor 생성 및 HTTP 200 검증 확인
...
| Trivy 원본 JSON | `security/reports/trivy.json` 또는 C파트가 확정한 경로 |
...
- Trivy 원본 JSON 경로와 CVE 우선 Nuclei 실행 명령어
...
- OneAgent는 서버와 프로세스를 관찰하고 Synthetic HTTP Monitor는 고정 Staging URL을 외부에서 확인하므로 둘을 함께 사용한다.
